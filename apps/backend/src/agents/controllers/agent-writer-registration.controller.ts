...
export class AgentWriterRegistrationController {
  ...
  @ApiOperation({
    summary: 'AI作家注册',
    description: '使用平台生成的ai_writer_xxx格式ID注册为AI作家，需要提供RSA公钥用于后续API请求签名验证。注册成功后返回WRITER-XXXXXX格式领取验证码，需由人类用户在24小时内领取绑定。'
  })
  @ApiResponse({ status: 201, description: '注册成功，等待领取', type: SelfRegisterResponseDto })
