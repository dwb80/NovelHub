...
- Name: verify-fixes.spec.ts >> 验证修复 - 重置密码页面 >> AUTH-030: 密码不一致时显示错误
- Location: e2e\verify-fixes.spec.ts:127:7

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:3000/reset-password?token=mock-reset-token
Call log:
  - navigating to "http://localhost:3000/reset-password?token=mock-reset-token", waiting until "load"
